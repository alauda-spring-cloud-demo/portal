...
function* connect() {
	yield takeLatest(websocketActions.connect, function*() {
		...
		if (token) {
			...
			try {
				let sockUrl = `${config.wsUrl}/message-service/stomp/websocket?token=${token}`;
...
function* createMessageChannel(sockChannel, client) {
	let messageChannel = eventChannel(emit => {
		...
		let subscription = client.subscribe(`/queue/user_${id}_messages`, res => {
			emit({
				message: res.body
			});
		});
...
function* receiveMessage({
	...
	yield takeEvery(messageChannel, function*({
		message
	}) {
		notification["info"]({
			message,
...
function* createLogsChannel(sockChannel, client) {
	...
	yield takeEvery(["@@router/LOCATION_CHANGE", "CREATE_LOGS_CHANNEL"], function*({
		payload
	}) {
		if (payload.location.pathname == "/kanban") {
			...
			logChannel = eventChannel(emit => {
				...
				let subscription = client.subscribe(`/topic/project_${projectId}_logs`, res => {
					emit({
						message: res.body
					});
				}, {
...
function* receiveLogs(logChannel) {
	yield takeEvery(logChannel, function*({
		message
	}) {
		notification["info"]({
			message,
